...
client.once('ready', async () => {
  ...
  const chSetterDesk = await safeCreate('#setter-desk', () =>
    guild.channels.create({
      ...
      type: ChannelType.GuildText,
      parent: catInbound.id,
      topic: 'Abdellah works leads, logs qualification notes, flags blockers.',
      permissionOverwrites: overwrites({
  ...
  const chWonHandoff = await safeCreate('#won-handoff', () =>
    guild.channels.create({
      ...
      topic: 'Closed-Won brief → triggers the Slack graduation. Patrick read-only.',
      permissionOverwrites: overwrites({
        closer: ['ViewChannel', 'SendMessages', 'ReadMessageHistory'],
  ...
  await safeCreate('Server map message', async () => {
    const msg = await chStartHere.send({
      content: [
        ...
        '`#setter-desk` — Abdellah qualifies, logs, flags',
        '',
        '**🎯 PIPELINE**',
        ...
        '',
        '## Roles',
        '`@Founder` (Jacques) — admin, also closes',
        '`@Closer` (Axel + Jacques) — pipeline, calls, floor',
        '`@Setter` (Abdellah) — inbound, qualify, tag deals',
        '`@Fulfillment` (Patrick) — read-only on #won-handoff only',
        '',
        '## The 5-minute rule',
        'First human touch on any 🆕 new lead in **< 5 minutes** during coverage hours.',
        'Bot fires an instant auto-ack. If 5 min pass → pings Abdellah. 10 min → Axel. 15 min → Jacques.',
      ].join('\n'),
    });
    await msg.pin();
    return msg;
